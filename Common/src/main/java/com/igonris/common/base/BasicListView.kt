...
class BasicListView(val items: List<String> = emptyList()): RecyclerView.Adapter<BasicListView.BLH>() {

    inner class BLH(view: View): RecyclerView.ViewHolder(view)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BLH {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.item_basic_list_view, parent, false)
        return BLH(view)
    }
